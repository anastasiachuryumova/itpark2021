...
import lesson14.contact.ContactWithOthers;
...
public class ContactRunner {
    ...
    private static void fillLinkedContacts (Collection<ContactWithOthers> contactList) {
        long now = System.currentTimeMillis();
        for (ContactWithOthers contact : contactList) {
            IntStream.range(0,LIMIT_OF_LINKED_CONTACTS).boxed()
                    .map(i -> randomElement(contactList)).forEach(contact.getLinkedContacts()::add);
        ...
        System.out.printf("Заполнение связанных контактов %s из %d элементов потребовало %.2f секунд\n",
               contactList instanceof List ? "списка" : "множества",
